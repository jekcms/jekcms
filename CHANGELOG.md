...
### Improved
- New positioning: a next-generation, smart CMS. The home page headline, the site tagline, the meta descriptions, the admin login screen and the license screen now say the same thing. jekcms was described everywhere as a blog CMS, which undersold it: the same core runs news sites, review and affiliate sites and company sites, and the wording no longer narrows it to blogs.
- Marketing site typography. Every long dash used as a separator was replaced with a plain hyphen across the site, including the changelog and the blog, so the copy reads the way a person writes rather than the way a machine does.
...
### Improved
- When a plugin is what is hiding your site, the panel now names it. Development-guard plugins send a `noindex` header and a blocking `robots.txt` so a site stays out of search while it is being built, and they are meant to be removed at launch. Until now the indexability check could see the effect but not the cause, and told you to go search your server for it - while the answer was sitting in your own plugin list. The check now scans the installed plugins, names the one responsible, and tells you to disable it if the site is ready (and that nothing is wrong if it is not).
...
- The new finding also tells you where to look. jekcms only sends an `X-Robots-Tag` header in maintenance mode and on a few internal endpoints, so if you see one on a normal page while maintenance is off, it is not coming from the software itself - which is exactly why searching the files that ship with jekcms never finds it. The panel now says this plainly and tells you where such a header can actually come from: a file that is not part of the package (your environment config, a `.user.ini` or an `auto_prepend_file` directive), or a layer sitting in front of your site. It gives you the command to run on your own server instead of leaving you to hunt for a line of code that is not there.
